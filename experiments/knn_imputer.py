...
from pairwise import _NAN_METRICS
...
class KNNImputer(_BaseImputer):
    ...
    ncat: numpy array
        ...
        This was not an attribute in the original version.
    References
    ----------
    * Olga Troyanskaya, Michael Cantor, Gavin Sherlock, Pat Brown, Trevor
    ...
    def __init__(self, missing_values=np.nan, n_neighbors=5,
                 weights="uniform", metric="nan_cat_euclidean", copy=True,
                 add_indicator=False, ncat=None):
        super().__init__(
    ...
    def _calc_impute(self, dist_pot_donors, n_neighbors,
        ...
        if (self.metric == 'nan_cat_euclidean') and (self.ncat is not None):
            if self.ncat[col] > 1:
                ...
                res = np.ma.average(donors_mask, axis=1, weights=weight_matrix).data
        else:
            res = np.ma.average(donors_mask, axis=1, weights=weight_matrix).data
        return res
            ...
            for col in range(X.shape[1]):
                ...
                if all_nan_receivers_idx.size:
                    if self.ncat is None:
                        ...
                        col_stat = mode(self._fit_X[:, col][~mask_fit_X[:, col]]).mode
                    else:
                        col_stat = np.ma.array(self._fit_X[:, col],
                                               mask=mask_fit_X[:, col]).mean()
