# This is the MissForest implementation from missingpy with a few changes
# on how the predictors are learned. More specifically, we do not retrain
# the predictors on the data to be imputed because, in our pipeline, this
# would "leak" information from the test data and corrupt the results.
# All the credit should go to the original authors (see below). """
...
class MissForest(BaseEstimator, TransformerMixin):
    ...
    * [3] https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.
    ...
    # This is the original imputation function as implemented in missingpy
    def _miss_forest_orig(self, Ximp, mask):
    ...
    # This is the updated version which does not retrain the predictors
    def _miss_forest(self, Ximp, mask):
        ...
        while (
            ...
            for s in misscount_idx:
                ...
                if self.cat_vars_ is not None and s in self.cat_vars_:
                    # 7. predict ymis(s) using xmis(x)
                    ymis = self.predictors[s].predict(xmis)
                    # 8. update imputed matrix using predicted matrix ymis(s)
                    Ximp[mis_rows, s] = ymis
                else:
                    # 7. predict ymis(s) using xmis(x)
    ...
    def fit(self, X, y=None, cat_vars=None):
        ...
        # Here is the key difference from other MissForest implementations.
        # We build the random forest regressor/classifier on the training data only.
        # The imputation is still done iteratively in the transform function,
        # but we do not update the random forest because we want to avoid using
        # test data information on the imputation process, as this would
        # corrupt our experiments.
        ...
        # If there are missing values in the training data
        if mask.sum() > 0:
            # Input the data with the original miss forest algorithm
            X = self._miss_forest_orig(X, mask)
        ...
        for col in col_index:
            if self.cat_vars_ is not None and col in self.cat_vars_:
                self.predictors.append(clone(rf_classifier))
                self.predictors[col].fit(X=np.delete(X, col, axis=1), y=X[:, col].ravel())
            else:
                self.predictors.append(clone(rf_regressor))
                self.predictors[col].fit(X=np.delete(X, col, axis=1), y=X[:, col].ravel())
        return self
