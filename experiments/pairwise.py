...
# This code has been adapted only slightly to handle categorical variables.
...
def nan_cat_euclidean_distances(X, Y=None, squared=False,
                                missing_values=np.nan, copy=True, ncat=None):
    """
    Similar to `nan_euclidean_distances` but computes the hamming distance of
    ...
    ncat: numpy array
        ...
        Variables with ncat > 1 are considered categorical.
    Returns
    -------
    distances : array, shape (n_samples_1, n_samples_2)
    >>> from pairwise import nan_cat_euclidean_distances, nan_euclidean_distances
    >>> nan = float("NaN")
    >>> X = [[0, 1, 1], [1, nan, 3]]
    >>> ncat = np.array([1, 1, 4])
    >>> nan_cat_euclidean_distances(X, X, ncat=ncat, squared=True) # distance between rows of X
    array([[0.        , 3],
           [3, 0.        ]])
    >>> nan_euclidean_distances(X, X, squared=True) # regular function
    array([[0.        , 7.5],
           [7.5, 0.        ]])
    """
    ...
    # set categorical values to zero
    if (ncat is not None):
        mask_ncat = ncat>1
        if np.sum(mask_ncat) > 0:
            X[:, mask_ncat] = 0
            Y[:, mask_ncat] = 0
...
#         distances = euclidean_distances(X, Y, squared=True)
    ...
    if (ncat is not None) and (np.sum(mask_ncat)) > 0:
        distances += cat_dist(X_orig[:, mask_ncat], Y_orig[:, mask_ncat])
...
PAIRWISE_DISTANCE_FUNCTIONS = {
    ...
    'nan_cat_euclidean': nan_cat_euclidean_distances
}
...
_VALID_METRICS = ['euclidean', 'l2', 'l1', 'manhattan', 'cityblock',
                  ...
                  'nan_euclidean', 'nan_cat_euclidean', 'haversine']

_NAN_METRICS = ['nan_euclidean', 'nan_cat_euclidean']
